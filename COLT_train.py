...
from models.AITM_wo_multi import COLT
from models.COLT import COLT as COLT_origin

def get_cmd():
    parser = argparse.ArgumentParser()
    parser.add_argument("-g", "--gpu", default="3", type=str, help="which gpu to use")
    parser.add_argument("-d", "--dataset", default="ToolBenchG3", type=str, help="which dataset to use, options:ToolLens,ToolBench")
    parser.add_argument("-m", "--model", default="AITM", type=str, help="which model to use, options: COLT")
    ...
    parser.add_argument("-nei", "--nei", default="10", type=int, help="num of neighbours to use,max number is 10")
    parser.add_argument("-att", "--att", default="none", type=str, help="none or simple or multihead")
    parser.add_argument("-origin", "--origin", default="False", type=str, help="none or simple or multihead")
    parser.add_argument("-threshold", "--threshold", default="0.8", type=float, help="none or simple or multihead")
...
def main():
    ...
    assert paras["model"] in ["COLT","AITM","new_AITM"], "Pls select models from: COLT"
    ...
    conf["threshold"] = paras['threshold']
    ...
    for base_model_name, lr, l2_reg, tool_level_ratio, scene_level_ratio, scene_agg_ratio, embedding_size, num_layers, c_lambda, c_temp in \
        ...
        if conf["attention_method"] == "none":
            ...
            settings += [str(c_lambda), str(c_temp), str(conf["num_neighbour"]), str(conf['attention_method']) ,str(conf["origin"]),str(conf['threshold'])]
        ...
        if conf["attention_method"] == "none":
            log_path = log_path + "/" + setting + "_COLT_origin"
        else:
            #setting += "_q_"+ conf['q_enable'] + "_t_" + conf['t_enable'] + '_3'
            log_path = log_path + "/" + setting  + "_COLT_attention_no_multi"
        ...
        if conf['model'] == 'AITM':
            if conf["attention_method"] == "none":
                model = COLT_origin(conf, dataset.graphs,test_list).to(device)
            else:
...
def test(model, dataloader, conf):
    ...
    for i,(queries, ground_truth_u_i) in enumerate(dataloader):
