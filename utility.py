...
class Datasets():
    def __init__(self, conf):
        ...
        self.num_queries, self.num_scenes, self.num_tools, self.num_neg_queries = self.get_data_size()
        #self.unseen_tool = unseen_tool
        ...
        neg_train = self.num_neg_queries*9//10
        neg_test = self.num_neg_queries-neg_train
        self.neg_train_queries = torch.arange(neg_train, dtype=torch.long).unsqueeze(dim=1)
        self.neg_test_queries = torch.arange(neg_train,self.num_neg_queries, dtype=torch.long).unsqueeze(dim=1)
        ...
        self.neg_train_loader = DataLoader(self.neg_train_queries, batch_size= neg_train*batch_size_train//len(self.scene_train_data), shuffle=True, num_workers=10, drop_last=True)
        ...
        self.neg_test_loader = DataLoader(self.neg_test_queries, batch_size=neg_test*batch_size_test//len(self.scene_test_data), shuffle=False, num_workers=20)
    ...
    def get_qt(self, task):
        if task == "test":
            with open(os.path.join(self.path, self.name, 'query_tool_{}_2.txt'.format(task)), 'r') as f:
